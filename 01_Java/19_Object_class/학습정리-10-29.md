...
#### 작성: 
	Ojbect의 하나의 함수이고, 
		파라메터는 : Object obj
		리    턴 : boolean 
		동    작 : 자신과 입려받은 참조변수의 값(주소)을 비교후 같으면 true 리턴한다.
...
#### 작성:
	String 클래스에서는, Object의 equals 함수를 오버라이드 하여 사용하고 있다.
		...
		동작 : 각각의 byte의 length 모두비교하여 
				하나라도 틀린 것이 있으면 false를 리턴 		----->(1)
	
		이유 : Oject 클래스의 equals 함수는 참조변수의 값만 비교를 하므로
			같은 문자열이 들어 있는 서로 다른 객체를 비교시했을때, 
			false가 리턴되므로  equals 함수를 
			(자신과 입력객체의 문자열이 같으면 true를 리턴되도록)
			오버라이딩 하여 사용한다.
